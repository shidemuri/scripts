...
game:GetService("StarterGui"):SetCore("SendNotification",{
    Title = "Prison Life Gale Fighter",
    Text = [[FE Gale Fighter mod by padero#0402
protect yo remote events next time dumb Aesthetical]],
    Duration = 5
})

local CDDF = {}
local DamageFling = function(DmgPer)
    if DmgPer.Name == game.Players.LocalPlayer.Name then return end
    if attack == false then return end
    if taunt == true then return end
    --if StateMover == false then return end
	CDDF[DmgPer] = true; StateMover = false
	for _=1,5 do
        game.ReplicatedStorage.meleeEvent:FireServer(game.Players[DmgPer.Name])
	end
...
cooldown = false
		...
		if key == 'q' and attack == false then attack = true attackthree() attack = false end
        if key == 'g' and attack == false then attack = true attackfour() attack = false end
		end)
